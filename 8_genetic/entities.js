function Vehicle(){
	...
	this.obstAttraction=random(minObstacleAcc,maxObstacleAcc);
	...
	//SCORE PARAMETERS
	this.health=1000;
	this.healthAtTerminatingState=-7;
	...
	this.calculateClosestToTarget=function(){
		...
		//console.log("DISTANCE TO TARGET",distance.mag())
	...
	this.terminatingNow=function(){
		...
		var distance=p5.Vector.sub(this.pos, this.target.pos);
		if(distance.mag()<20){ //TARGET REACHED

			this.closestEverDistanceToTarget=1;//HACK LETS SEE
			this.terminated=true;
			this.healthAtTerminatingState=1000;
			this.timeTaken=generationTime;// GENERATIONTIME VARIABLE FROM SKETCHJS, DONT REALLY KNWO HOW TO CODE In JAVASCRIPT
			this.pushDataToMatingPool();
			}
		else if(this.health<0){// COLLISSON WITH BOXES
			this.terminated=true;
			this.healthAtTerminatingState=-10000;
			this.timeTaken=0;// SINCE ITS BAD TO DIE
			this.pushDataToMatingPool();
		}
		else if (generationTime>2000){
			
			this.healthAtTerminatingState=1000;
			this.timeTaken=generationTime;
			this.terminated=true;
			this.timeTaken=generationTime;// GENERATIONTIME VARIABLE FROM SKETCHJS, DONT REALLY KNWO HOW TO CODE In JAVASCRIPT
			this.pushDataToMatingPool();
		}
	...
	this.pushDataToMatingPool=function(){
